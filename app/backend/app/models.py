...
from sqlalchemy import String,Text,Date,DateTime,Boolean,BigInteger,ForeignKey,UniqueConstraint
...
class DocumentVersion(Base):
    __tablename__="document_versions"; __table_args__=(UniqueConstraint("document_id","version"),); id:Mapped[str]=mapped_column(String(36),primary_key=True,default=uid); document_id:Mapped[str]=mapped_column(ForeignKey("documents.id"),index=True); version:Mapped[int]; kind:Mapped[str]=mapped_column(String(50),default="original"); original_name:Mapped[str]=mapped_column(String(500)); stored_name:Mapped[str]=mapped_column(String(500),unique=True); mime_type:Mapped[str|None]=mapped_column(String(200)); size:Mapped[int]=mapped_column(BigInteger); sha256:Mapped[str]=mapped_column(String(64),index=True); ocr_status:Mapped[str]=mapped_column(String(30),default="pending"); ocr_text:Mapped[str|None]=mapped_column(Text); created_at:Mapped[datetime]=mapped_column(DateTime,default=datetime.utcnow)
class Tag(Base):
