...
from .models import SessionToken,ShareLink,CheckResult,Audit,ConnectorSample,ComplianceResult,Notification
while True:
 try:
  with SessionLocal() as db:
   ...
   db.execute(delete(ComplianceResult).where(ComplianceResult.checked_at<now-timedelta(days=90)))
   db.execute(delete(Notification).where(Notification.created_at<now-timedelta(days=180),Notification.read==True))
