...
invp=Path(sys.argv[1]);root=Path(sys.argv[2])
try: devices=json.loads(invp.read_text())
except: devices=[]
...
while True:
    ...
    uf=f"{did}.user";pf=f"{did}.password";ef=f"{did}.enable"
    (root/uf).write_text(user);(root/pf).write_text(pwd)
    if ena:(root/ef).write_text(ena)
    d={"id":did,"name":name,"host":host,"port":port,"device_type":dtype,"site":site,"role":role,"enabled":True,
       "username_file":uf,"password_file":pf}
    if ena:d["enable_secret_file"]=ef
    devices=[x for x in devices if str(x.get("id"))!=did];devices.append(d)
invp.write_text(json.dumps(devices,ensure_ascii=False,indent=2))
for p in root.iterdir():
    if p.is_file():p.chmod(0o600)
print(f"Saved {len(devices)} device(s).")
PY
